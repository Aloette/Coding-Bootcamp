...
public class StudentDAO {
	...
	public void open() throws SQLException {
		...
		try {
			...
			// object (con)
			con = DriverManager.getConnection("jdbc:mysql://localhost:3306/bootcamp", "root", "1234");
		} catch (Exception e) {
			con = null;
	...
	public Student findStudentByAM(String am) throws Exception {
		...
		try {
			...
			//your code here 
			String sql= "select * from student_table where am= '"+am+"';";
			PreparedStatement stmt = con.prepareStatement(sql);
			ResultSet rs = stmt.executeQuery();
			while(rs.next()){
				if(rs!=null){
					student.setAm(rs.getString(1));
